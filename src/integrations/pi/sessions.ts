...
export function normalizePiModel(provider: string, model: string): string {
  const hosted = model.match(
    new RegExp(`^accounts/${provider}/(?:models|routers)/(.+)$`),
  );
...
export function parsePiSession(text: string): PiUsageRecord[] {
  ...
  for (const line of text.split("\n")) {
    ...
    const record: PiUsageRecord = {
      id: `${sessionId}:${row.id}`,
      sessionId,
      provider: normalizePiProvider(message.provider),
      model: normalizePiModel(message.provider, message.model),
      createdMs,
      billingMode: piBillingMode(message.provider),
      tokens: {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) await walkJsonl(path, sinceMs, out);
      ...
      try {
        ...
        out.push(
          ...parsePiSession(await readFile(path, "utf8")).filter(
            (r) => r.createdMs >= sinceMs,
          ),
        );
      } catch {
        /* skip files being replaced while Pi writes */
      }
    }
  }
}
...
const emptyTotals = (at: string): SummerTotals => ({
  since: at,
  prepaidUsd: 0,
  usageUsd: 0,
  usageRealUsd: 0,
...
const unpriceable = (e: unknown) => {
  ...
  return (
    ...
    /not found in models\.dev/i.test(
...
export async function processPiSessions(
  ...
  for (const r of records) {
    ...
    try {
      ...
      if (r.billingMode === "api") usageRealUsd += value;
      else usageSubUsd += value;
      ...
      if (unpriceable(error)) {
        seen[r.id] = r.createdMs;
        changed = true;
      } else
        log.warn({
          action: "pi_usage_track_failed",
          id: r.id,
  ...
  const pruned = Object.fromEntries(
    Object.entries(seen).filter(([, ms]) => ms >= cutoff),
  );
  ...
  await writeState({
    ...
    totals: {
      ...totals,
      usageUsd: totals.usageUsd + usageUsd,
      usageRealUsd: totals.usageRealUsd + usageRealUsd,
      usageSubUsd: totals.usageSubUsd + usageSubUsd,
...
export async function gatherPiRecords(opts: { since?: Date; until?: Date }) {
  ...
  return (await collectPiRecords(since ?? 0)).filter(
    (r) =>
      (since == null || r.createdMs >= since) &&
      (until == null || r.createdMs < until),
  );
}
