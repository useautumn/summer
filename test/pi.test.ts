...
import {
  ...
  normalizePiProvider,
  parsePiSession,
} from "../src/integrations/pi/sessions.ts";
...
const fixture = [
  ...
  line({
    ...
    message: {
      ...
      model: "gpt-5.4",
      timestamp: Date.parse("2026-07-10T10:00:03Z"),
      usage: { input: 10, output: 4, cacheRead: 20, cacheWrite: 5 },
    },
  }),
  "partial {",
  line({
...
test("parsePiSession extracts assistant usage and prefers message timestamp", () => {
  ...
  expect(records[0]).toMatchObject({
    ...
    createdMs: Date.parse("2026-07-10T10:00:03Z"),
  });
...
test("normalizePiProvider only aliases known broker ids", () => {
  ...
  expect(
    normalizePiModel("fireworks", "accounts/fireworks/routers/kimi-k2p5-turbo"),
...
test("collectPiRecords honors PI_CODING_AGENT_SESSION_DIR", async () => {
  ...
  try {
    expect(await collectPiRecords(0)).toHaveLength(1);
  } finally {
...
test("collectPiRecords filters old messages inside recently-touched sessions", async () => {
  ...
  try {
    expect(
      (await collectPiRecords(Date.now() - 60_000)).map((r) => r.id),
    ).toEqual(["S-1:recent"]);
  } finally {
    delete process.env.PI_CODING_AGENT_SESSION_DIR;
  }
